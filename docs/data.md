...
The synthetic data follows a sparse linear generative model:

$$\mathbf{y} = A\mathbf{z}$$

where $A \in \mathbb{R}^{m \times n}$ is a mixing matrix with normalised columns and $\mathbf{z} \in \mathbb{R}^n$ is a $k$-sparse latent code (exactly $k$ non-zero entries).
...
|---------|---------------|----------|
| **A** | $z_0$ active + $(k-1)$ from IID pool | IID training |
| **B** | $k$ from IID pool (no $z_0$) | IID training |
| **C** | $z_0$ active + $(k-1)$ from OOD pool | OOD test |

The IID and OOD pools are disjoint subsets of $\{z_1, \ldots, z_{n-1}\}$, so OOD samples contain $z_0$ paired with latents *never seen active alongside it during training*.
...
train, val, ood, A = generate_datasets(
    ...
    k=3,              # sparsity level
    n_samples=2000,   # IID training samples
...
|-----------|-------------|---------|
| `num_latents` | $n$ — number of latent sources | — |
| `k` | number of simultaneously active sources | — |
| `n_samples` | number of IID training samples | — |
