...
Z_iid, Y_iid, labels_iid = train
Z_ood, Y_ood, labels_ood = ood
```

### `generate_data(n, k, m, n_samples, rng=None)`

Lower-level generator. Returns `((Z_iid, Y_iid, label_iid), (Z_ood, Y_ood, label_ood), A)`.
...
### Sparse coding — `models.sparse_coding`
...
Train sparse coding with method specified in `cfg.method` (`"fista"`, `"ista"`, `"lista"`, `"direct"`). Returns dict with codes, learned dictionary, and training history.
...
cfg = SparseCodingConfig(
    ...
    method="fista", lam=0.1, n_iter=100,
)
```

#### `LISTA(n_obs, n_latent, n_unroll=16)`

Learned ISTA encoder. Call `init_from_dictionary(D)` to initialise from analytical ISTA, then `forward(x)` for fast inference.
...
### Linear probe — `models.linear_probe`
